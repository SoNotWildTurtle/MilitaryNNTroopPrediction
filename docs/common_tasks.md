...
Then open `ci_artifacts/local-ci/release-bundle-index.html` and check `reviewer-handoff.md` first when sharing context with another maintainer. Check `reviewer-handoff-validation.json` next when downstream automation rejects a bundle; it is produced by `scripts/validate_reviewer_handoff.py` and reports the exact contract errors. Check `operator-runbook-index.md` when an operator or reviewer needs the generated map of safe commands, docs, artifacts, and first steps. Check `triage-summary.md` when a run failed. It summarizes failing health checks, missing expected artifacts, and the narrow target to rerun, such as `make doctor`, `make test`, `make ci-report`, `make validate-handoff`, `make openapi`, `make examples`, `make dashboard`, `make previews`, `make manifest`, `make release-notes`, `make reviewer-handoff`, `make triage-summary`, or `make runbook-index`.
...
make runbook-index ARTIFACT_DIR=ci_artifacts/operator-map
make openapi ARTIFACT_DIR=ci_artifacts/api-contract-review
```

Open `ci_artifacts/release-bundle-index.html` first when reviewing a generated diagnostics bundle. It links the reviewer handoff, health report, release notes, triage summary, operator runbook index, OpenAPI contract, synthetic API examples, static dashboard mockup, HTML previews, and manifest from one dependency-free page. The companion guide at `docs/release_bundle_review.md` gives the quick reviewer handoff flow.

`reviewer-handoff.md` includes a normalized review status, a copyable summary, missing expected outputs, missing key artifacts, and the recommended narrow rerun command. Use that file when sending a diagnostics bundle to another maintainer because it summarizes whether the bundle is ready, needs warning review, or needs attention before deeper inspection. `operator-runbook-index.md` complements it with a generated command map, documentation map, artifact review map, first safe path, and safe-scope reminder for operators and maintainers who are orienting themselves inside the repository. `reviewer-handoff-validation.json` and `reviewer-handoff-validation.txt` record the dependency-free validation result from `scripts/validate_reviewer_handoff.py` so reviewers and automation can confirm the machine-readable handoff follows `docs/reviewer_handoff_contract.md` before consuming it.
...
| `make runbook-index` | Export an operator-facing command, documentation, artifact, first-step, and safe-scope index. |
