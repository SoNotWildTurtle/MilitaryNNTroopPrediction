...
Open `ci_artifacts/operator-digest.md` first when reviewing a generated diagnostics bundle. It summarizes the current review status, release health status, indexed artifact count, blocking reasons, copyable summary, and the top narrow rerun command. Then open `ci_artifacts/release-bundle-index.html` for the full dependency-free landing page. The companion guide at `docs/release_bundle_review.md` gives the quick reviewer handoff flow.
...
| `make ci-report` | Build the same diagnostics bundle used by CI artifacts, including handoff validation outputs and the operator digest. |
...
| `make operator-digest` | Export a concise operator-first Markdown/JSON digest with status, next step, blocking reasons, and top rerun actions. |
