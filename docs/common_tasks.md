...
That keeps local and pull-request validation aligned. When CI fails, reproduce the run locally with the same command, then open `ci_artifacts/operator-artifact-guide.md` or `ci_artifacts/release-bundle-index.html` first to inspect generated health reports, release notes, reviewer handoff notes, triage guidance, API contracts, examples, previews, manifests, and handoff validation results.

After it completes, open `ci_artifacts/operator-artifact-guide.md` first when you want plain-language guidance about which artifact to inspect. Open `ci_artifacts/release-bundle-index.html` when you want a linked static landing page. Use `docs/release_bundle_review.md` as the reviewer checklist before sharing or summarizing the bundle.
...
Open `ci_artifacts/operator-artifact-guide.md` first when you want a short operator menu. It explains the recommended first step, health counts, missing expected outputs, and what each key artifact is for. Open `ci_artifacts/release-bundle-index.html` when reviewing the generated diagnostics bundle through a linked static page. The companion guide at `docs/operator_artifact_guide.md` explains the operator menu, and `docs/release_bundle_review.md` gives the quick reviewer handoff flow.
...
| `make ci-report` | Build the same diagnostics bundle used by CI artifacts, including handoff validation outputs and the operator artifact guide. |
...
| `make operator-artifact-guide` | Export an operator-friendly artifact menu with recommended first step and audience-specific guidance. |
