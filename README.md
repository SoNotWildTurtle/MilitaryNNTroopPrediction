...
With those settings in place, choose “Object drilldown & alerts” inside the dashboard to build an OBIEE-style report, review average movement speeds for armor, aircraft, and drone units, and send the summary to selected phone numbers via Twilio.
...
- `cli/method_cohesion_report.py` – show classes flagged by multiple analysis methods (now including streak and interarrival checks), print highlights for each contributing method, and display a pairwise overlap matrix. Power users can call `analysis_method_cohesion(extra_specs=[...])` to blend in additional analytics beyond the defaults.
 - `cli/doctrine_movement_report.py` – drill into time-bucketed movement stats per doctrine for units, groups, or battalions.
 - `cli/method_cohesion_report.py` – show classes flagged by multiple analysis methods (now including streak and interarrival checks), print highlights for each contributing method, and display a pairwise overlap matrix. Power users can call `analysis_method_cohesion(extra_specs=[...])` to blend in additional analytics beyond the defaults.
...
python -m app.cli.doctrine_movement_report  # doctrine-aware movement drilldown
