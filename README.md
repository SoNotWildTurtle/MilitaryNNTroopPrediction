...
A LIDAR module provides electromagnetic-based detection and a sensor fusion helper merges camera and LIDAR results for higher confidence.
...
- `cli/fusion_report.py` – run camera and LIDAR detection for troops, vehicles, and drones and display fused results; accepts `--image` and `--pointcloud` to skip prompts.
...
python -m app.cli.fusion_report --image sample.jpg --pointcloud sample.pcd  # fuse camera and LIDAR detections for troops, vehicles, and drones
