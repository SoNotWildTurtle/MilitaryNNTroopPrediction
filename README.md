...
- `detection/feature_fused_identifier.py` – classify images using fused color, HOG, and edge features; train via `python -m app.cli.train_feature_fused_identifier` and run predictions with `python -m app.cli.feature_fused_classify`.
...
- `cli/dashboard.py` – interactive operator dashboard to run pipeline tasks plus
    ...
    activity, confidence summaries, prediction correlations, and sensor
    reliability weights. Each tool accepts a start/end date range or lookback
    hours. The training page handles self-reinforcement, auto training, and
    hyperparameter searches. The main screen also streams drone feeds, captures
