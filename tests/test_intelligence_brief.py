...
def test_automation_force_projection_requires_signals():
    assert intel_brief._derive_automation_force_projection({}) is None


def test_automation_force_projection_flags_manual_bridge():
    brief: Dict[str, Any] = {
        "automation_strategic_convergence": {
            "status": "manual_bridge",
            "recommended_actions": ["Coordinate manual sync"],
            "cross_domain_tracks": [
                {
                    "name": "Strategic automation bridge",
                    "lead": "Strategic HQ",
            ...
            "strategic_channels": ["Strategic Net"],
            "coalition_partners": ["Allied HQ"],
            "ukrainian_operator_prompts": ["Узгодити дії з союзниками."],
        },
        "automation_supreme_command": {
            "status": "manual_override",
            "command_tracks": [
                {
                    "name": "National control",
                    "lead": "National cell",
                    "mode": "manual",
                    "readiness": "guarded",
                    "window_hours": 3.0,
            ...
            "command_nodes": ["Kyiv HQ"],
            "integration_channels": ["Command Net"],
            "recommended_actions": ["Manual release only"],
        },
        "automation_theater_command": {
            "status": "manual_bridge",
            "command_tracks": [
            ...
            "command_channels": ["Theatre Net"],
            "coordinating_theaters": ["East"],
            "recommended_actions": ["Coordinate theatre bridge"],
        },
        "automation_joint_operations": {
            "status": "manual_bridge",
            "joint_operation_tracks": [
                {
                    "name": "Coalition corridor",
                    "lead": "Coalition liaison",
                    "mode": "manual",
                    "readiness": "watch",
                    "window_hours": 5.0,
            ...
            "support_cells": ["Coalition support"],
            "integration_channels": ["Coalition Net"],
            "coalition_partners": ["Coalition HQ"],
            "recommended_actions": ["Bridge coalition release"],
        },
        "automation_battle_management": {
            "status": "manual_bridge",
            "coordination_tracks": [
            ...
            "coordination_cells": ["Battle cell"],
            "mission_channels": ["Battle Net"],
            "recommended_actions": ["Hold automation"],
        },
        "automation_mission_control": {
            "status": "manual_control",
            "mission_channels": ["Mission Net"],
            "recommended_actions": ["Log manual oversight"],
            "handoff_requirements": ["Notify mission control"],
        },
        "automation_overwatch": {
            "status": "manual_watch",
            "monitoring_channels": ["Overwatch Net"],
            "recommended_actions": ["Increase monitoring"],
            "ukrainian_operator_prompts": ["Тримайте додаткове чергування."],
        },
        "automation_guardrails": {
            "status": "locked_down",
            "monitoring_channels": ["Guardrail Log"],
            "recommended_actions": ["Reset guardrails"],
            "ukrainian_operator_prompts": ["Зареєструйте ручні виключення."],
        },
        "automation_playbook": {
            "status": "manual_override",
            "automation_tasks": [
            ...
            "automation_channels": ["Automation Net"],
            "recommended_actions": ["Manual confirm"],
        },
        "automation_autonomy": {
            "status": "manual_guarded",
            "ukrainian_safeguards": ["Підтвердіть ручний контроль."],
        },
        "automation_failsafes": {
            "status": "manual_recovery",
            "fallback_channels": ["Fallback Net"],
            "recommended_actions": ["Manual fallback"],
            "ukrainian_operator_prompts": ["Перевірте резервні канали."],
        },
        "automation_validation": {
            "status": "manual_review",
            "recommended_actions": ["Manual validation"],
        },
        "automation_deployment": {
            "status": "hold",
            "recommended_actions": ["Pause deployments"],
            "deployment_tracks": [
                {
                    "name": "Deployment batch",
                    "lead": "Deployment cell",
                    "mode": "manual",
                    "readiness": "hold",
                    "window_hours": 3.5,
                }
        ...
        "frontline_support": {
            "status": "critical",
            "priority_units": ["92-га бригада"],
            "support_corridors": ["Logistics corridor"],
            "drivers": ["Frontline surge"],
            "ukrainian_operator_notes": ["Узгодьте з бригадою."],
        },
        "resource_sustainment": {
            "status": "surge",
            "resource_needs": ["Fuel"],
            "allocation_plan": [
                {"resource": "Ammunition", "quantity": 4, "window_hours": 6.0}
            ],
            "support_dependencies": ["Bridge repair"],
            "recommended_actions": ["Surge convoys"],
        },
        "support_priorities": {
            "status": "mobilise",
            "priorities": [{"name": "Telecom support", "support_window_hours": 4.0}],
        },
        "response_readiness": {
            "level": "critical",
            "support_window_hours": 2.0,
            "recommended_actions": ["Add shifts"],
        },
        "response_pressure": {
            "status": "critical_backlog",
            "estimated_clearance_hours": 5.0,
            "recommended_actions": ["Clear backlog"],
        },
        "mission_assurance": {
            "status": "at_risk",
            "recommended_actions": ["Brief leadership"],
        ...
        "operational_resilience": {
            "status": "fragile",
            "recommended_actions": ["Add reserves"],
        },
        "operational_continuity": {
            "status": "constrained",
            "recommended_actions": ["Stabilise comms"],
        },
        "operational_recovery": {
            "status": "recover",
            "recommended_actions": ["Continue recovery"],
        },
        "operational_transformation": {
            "transformation_score": 50.0,
            "recommended_actions": ["Accelerate training"],
        },
        "operational_governance": {
            "governance_score": 55.0,
            "next_review_hours": 3.0,
            "recommended_actions": ["Convene council"],
        },
        "operational_risks": {
            "severity_score": 88.0,
            "risks": ["Automation backlog"],
        },
    }

    payload = intel_brief._derive_automation_force_projection(brief)
    assert payload is not None
    assert payload.get("status") in {"manual_override", "projection_bridge"}
    assert payload.get("force_projection_score") is not None
    assert payload.get("force_packages")
    assert any("92-га" in pkg for pkg in payload.get("force_packages", []))
    prompts = payload.get("ukrainian_operator_prompts", [])
    assert any("у" in prompt.lower() for prompt in prompts)
    tracks = payload.get("projection_tracks", [])
    assert isinstance(tracks, list) and tracks
    assert any(track.get("source") for track in tracks)
    actions = payload.get("recommended_actions", [])
    assert any("manual" in action.lower() or "bridge" in action.lower() for action in actions)


def test_automation_force_projection_recognises_ready_state():
    brief: Dict[str, Any] = {
        "automation_strategic_convergence": {
            "status": "mission_ready",
            "recommended_actions": ["Share convergence briefing"],
            "cross_domain_tracks": [
                {
                    "name": "Strategic sync",
                    "lead": "Strategic HQ",
        ...
        "automation_supreme_command": {
            ...
            "command_nodes": ["Kyiv HQ"],
            "integration_channels": ["Command Net"],
            "recommended_actions": ["Broadcast readiness"],
        },
        "automation_theater_command": {
            "status": "synchronised_command",
            "command_tracks": [
            ...
            "command_channels": ["Theatre Net"],
            "coordinating_theaters": ["East"],
            "recommended_actions": ["Notify theatre"],
        },
        "automation_joint_operations": {
            "status": "coalition_ready",
            "joint_operation_tracks": [
                {
                    "name": "Coalition automation",
                    "lead": "Coalition liaison",
                    "mode": "autonomous",
                    "readiness": "ready",
                    "window_hours": 5.0,
            ...
            "integration_channels": ["Coalition Net"],
            "coalition_partners": ["Coalition HQ"],
            "recommended_actions": ["Share coalition plan"],
        },
        "automation_battle_management": {
            "status": "mission_ready",
            "coordination_tracks": [
            ...
            "mission_channels": ["Battle Net"],
            "recommended_actions": ["Execute battle automation"],
        },
        "automation_mission_control": {
            "status": "mission_ready",
            "mission_channels": ["Mission Net"],
            "recommended_actions": ["Broadcast mission control status"],
        },
        "automation_guardrails": {
            "status": "autonomous",
            "monitoring_channels": ["Guardrail Net"],
            "recommended_actions": ["Continue guardrail monitoring"],
        },
        "automation_playbook": {
            "status": "autonomous",
            "automation_tasks": [
            ...
            "automation_channels": ["Automation Net"],
            "recommended_actions": ["Scale automation"],
        },
        "automation_autonomy": {
            "status": "autonomous_ready",
            "ukrainian_safeguards": ["Відстежуйте автоматизацію."],
        },
        "automation_failsafes": {
            "status": "ready",
            "fallback_channels": ["Fallback Net"],
            "recommended_actions": ["Maintain failsafes"],
        },
        "automation_validation": {
            "status": "mission_ready",
            "recommended_actions": ["Log validation"],
        },
        "automation_deployment": {
            "status": "ready",
            "deployment_tracks": [
            ...
            "recommended_actions": ["Deploy automation"],
        },
        "frontline_support": {
            "status": "steady",
            "priority_units": ["93-тя бригада"],
            "ukrainian_operator_notes": ["Підтримуйте взаємодію."],
        },
        "resource_sustainment": {
            "status": "reinforced",
            "resource_needs": ["Batteries"],
            "allocation_plan": [
                {"resource": "Fuel", "quantity": 3, "window_hours": 4.5}
            ],
        },
        "support_priorities": {
            "status": "watch",
            "priorities": [{"name": "Telemetry cell"}],
        },
        "response_readiness": {
            "level": "steady",
            "support_window_hours": 6.0,
            "recommended_actions": ["Maintain rotations"],
        },
        "response_pressure": {
            "status": "steady",
            "estimated_clearance_hours": 2.0,
        ...
        "mission_assurance": {
            "status": "stabilise",
            "recommended_actions": ["Publish assurance"],
        },
        "operational_resilience": {
            "status": "resilient",
            "recommended_actions": ["Share resilience"],
        },
        "operational_continuity": {
            "status": "stable",
            "recommended_actions": ["Lock continuity"],
        },
        "operational_transformation": {
            "transformation_score": 82.0,
            "recommended_actions": ["Highlight success"],
        },
        "operational_governance": {
            "governance_score": 85.0,
            "next_review_hours": 8.0,
            "recommended_actions": ["Schedule review"],
        },
        "operational_risks": {
            "severity_score": 40.0,
        },
    ...
    assert payload is not None
    assert payload.get("force_projection_score") is not None
    assert payload.get("force_projection_score") >= 80
    assert payload.get("status") not in {"manual_override", "projection_bridge"}
    assert payload.get("projection_tracks")
    assert payload.get("projection_channels")
    assert any("бригада" in pkg for pkg in payload.get("force_packages", []))
    assert any("automation" in action.lower() for action in payload.get("recommended_actions", []))


def test_gather_intelligence_brief_adds_automation_force_projection(monkeypatch):
    payload = {
        "status": "projection_ready",
        "force_projection_score": 94.2,
        "projection_window_hours": 3.0,
        "force_packages": ["92-га бригада", "Signal cell"],
        "projection_tracks": [
            {
                "name": "Strategic lane",
                "lead": "Automation HQ",
        ...
        "projection_channels": ["Strategic Net", "Mission Net"],
        "recommended_actions": ["Coordinate projection briefing"],
    }

    monkeypatch.setattr(intel_brief, "_utcnow", lambda: datetime(2024, 8, 1, tzinfo=UTC))
    monkeypatch.setattr(intel_brief, "_recent_documents", lambda *args, **kwargs: [])
    monkeypatch.setattr(intel_brief, "_recent_clusters", lambda *args, **kwargs: [])
    monkeypatch.setattr(intel_brief, "meta_analysis", lambda hours: {"detections": {}})
    monkeypatch.setattr(intel_brief, "_derive_automation_strategic_convergence", lambda brief: {})
    monkeypatch.setattr(intel_brief, "_derive_automation_force_projection", lambda brief: payload)

    brief = intel_brief.gather_intelligence_brief(hours=6, activity_limit=5)

    projection = brief.get("automation_force_projection")
    assert projection == payload

    insight = brief.get("insights", {}).get("automation_force_projection", {})
    assert insight.get("status") == payload["status"]
    assert (
        insight.get("force_projection_score") == payload["force_projection_score"]
    )
    assert (
        insight.get("projection_window_hours")
        == payload["projection_window_hours"]
    )
    assert (
        insight.get("projection_track_count")
        == len(payload["projection_tracks"])
    )
    assert (
        insight.get("projection_channel_count")
        == len(payload["projection_channels"])
    )
...
def test_operator_dashboard_snapshot_flags_strain_and_prompts():
    ...
    alerts = dashboard.get("operator_alerts", [])
    assert alerts and any("backlog" in alert.lower() or "stale" in alert.lower() for alert in alerts)
    watch = dashboard.get("watch_items", [])
    assert watch and any("feed" in item.lower() for item in watch)
    checklist = dashboard.get("shift_checklist", [])
    assert checklist and any("override" in item.lower() or "ticket" in item.lower() for item in checklist)
    shortcuts = dashboard.get("utility_shortcuts", [])
    labels = {item.get("label") for item in shortcuts if isinstance(item, dict)}
    assert "Telemetry validation" in labels
    assert "Surge reroute" in labels
...
def test_operator_dashboard_snapshot_recognises_ready_state():
    ...
    assert not dashboard.get("operator_alerts")
    assert not dashboard.get("shift_checklist")
    shortcuts = dashboard.get("utility_shortcuts", [])
    assert shortcuts and any(
        shortcut.get("label") == "Operator dashboard" for shortcut in shortcuts if isinstance(shortcut, dict)
    )
...
def test_gather_intelligence_brief_includes_operator_dashboard(monkeypatch):
    payload = {
        ...
        "utility_shortcuts": [
            {"label": "Operator dashboard", "command": "python -m app.cli.dashboard", "description": "Launch"}
        ],
    }
    ...
    assert insight.get("utility_shortcuts") == len(payload["utility_shortcuts"])
    recs = brief.get("recommendations", [])
    assert any("telemetry" in rec.lower() for rec in recs)
