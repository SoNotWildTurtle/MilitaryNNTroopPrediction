...
.PHONY: help install-core install-optional configure doctor quickstart api test verify ci-triage ci-report openapi examples dashboard bundle-index previews manifest release-notes reviewer-handoff operator-session-plan validate-handoff triage-summary clean
...
ci-triage:
	...
	@printf '4. If the bundle is incomplete, inspect the generated handoff, its validation result, the operator session plan, and the triage summary, then rerun the narrow target:\n'
	...
	@printf '   make doctor | make test | make ci-report | make validate-handoff | make openapi | make examples | make dashboard | make previews | make manifest | make release-notes | make reviewer-handoff | make operator-session-plan | make triage-summary\n\n'
