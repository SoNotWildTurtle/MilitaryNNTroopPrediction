...
70.5. Ship a system check CLI that validates dependency installation, optional GPU tooling, and alert credentials. (done)
...
171.5. Surface a next-gen recommendations briefing that fuses detections and forecasts into priority, watch, sensor-refresh, intelligence, and data-quality tasks via CLI and dashboard options.
...
180. Strengthen movement predictor surety scoring with detailed history, fit-quality, and plausibility checks exposed in the prediction payload.
181. Deliver an alert automation GUI (with `/alerts` API) so operators can define troop/vehicle/drone rules that trigger SMS and email notifications. (done)
182. Provide a Textual control centre accessible via `cli/control_center.py` and the dashboard for aggregated recommendations, risk matrix insights, and speed summaries. (done)
183. Deliver an operational tactics overview that fuses detections, doctrine cues, and trajectory forecasts into posture/logistics/air guidance with CLI and dashboard access.
184. Provide a military campaign assessment that scores pressure, tempo, logistics, air activity, and attrition, with CLI access and dashboard integration.
185. Deliver a theatre outlook that fuses corridor dominance, axes of advance, tempo shifts, and counter-recommendations via `theater_assessment.py`, CLI tooling, and dashboard access.
