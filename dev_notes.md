...
- Setup scripts (setup.sh/start.sh) automate dependency install and pipeline startup. The setup script now installs OpenCV, scikit-image, Ultralytics, and CPU PyTorch by default, upgrading to CUDA builds automatically when a GPU is detected.
- cli/system_check.py provides a translated health check that reports missing dependencies and alert credentials with optional JSON output for automation.
...
- analysis/military_campaign.py blends detection counts with prediction tempo metrics to score frontline pressure, logistics health, air activity, and attrition risk for campaign briefings.
- cli/military_campaign_report.py retrieves recent data for an area and renders the campaign assessment with recommended counter-actions.
...
- `cli/control_center.py` launches a Textual control centre that aggregates next-gen recommendations, risk matrix bands, and per-object speed summaries, and the dashboard menu exposes it for quick access.
...
- next_gen_recommendations.py now compiles high-priority actions, watch items, sensor refresh tasks, intelligence follow-ups, and data-quality guidance from recent detections and forecasts; operators can run it via `cli/next_gen_recommendations.py` or through the dashboard analysis menu and use the `focus` payload for quick status tables.
- operational_tactics.py assembles a combined-arms posture brief by merging detections, doctrine tags, and trajectory predictions; `cli/operational_analysis_report.py` and the dashboard analysis submenu display posture, logistics, air activity, and suggested counter-actions.
...
- analysis/theater_assessment.py computes theatre-level corridor stats, axes of
  advance, tempo trends, and recommendations; operators can launch the Rich
  report via `cli/theater_assessment_report.py`, and the dashboard analysis
  submenu now exposes a "Theatre outlook" option alongside campaign reports.
