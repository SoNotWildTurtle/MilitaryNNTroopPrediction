...
- lidar_detector.py simulates LIDAR-based detection for troops, vehicles, and drones.
- sensor_fusion.py averages camera and LIDAR confidences; `detect_fused_objects` wraps both sensors and cli/fusion_report.py displays fused results. The CLI supports `--image` and `--pointcloud` arguments to skip prompts.
