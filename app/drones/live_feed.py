...
def stream(
    ...
    target_clf = load_unified_model(target_model_path) if classify_targets else None
    ...
    while True:
        ...
        if target_clf:
            for det in detections:
                res = classify_target(tmp_path, target_clf)
                det.update({"target": res["target"], "target_conf": res["confidence"]})
...
def _parse_args():
    ...
    parser.add_argument("--target-model", dest="target_model", help="Unified classifier path", default=None)
    parser.add_argument("--area", default="unknown", help="Area identifier")
    parser.add_argument("--classify", action="store_true", help="Run unified target classifier")
...
def main() -> None:
    ...
    stream(
        ...
        classify_targets=args.classify,
    )
