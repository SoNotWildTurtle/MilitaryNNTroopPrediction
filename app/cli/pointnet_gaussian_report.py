"""CLI to match point clouds using a PointNet-Gaussian model."""
...
from ..analysis import image_to_pointcloud3d, rank_pointnet_gaussian
...
def run_pointnet_gaussian_report() -> None:
    parser = argparse.ArgumentParser(
        description=_t("Match point clouds with a PointNet-Gaussian model")
    )
    ...
    parser.add_argument(
        ...
        help=_t("Trained PointNet encoder weights"),
    )
    parser.add_argument(
        "--stats",
        default="pointnet_gaussian_model.json",
        help=_t("Gaussian stats JSON"),
    )
    parser.add_argument(
        "--top", type=int, default=3, help=_t("Number of top classes to display")
    )
    args = parser.parse_args()

    image_cloud = image_to_pointcloud3d(args.image)
    sensor_cloud = _load_sensor_cloud(args.sensor)
    results = rank_pointnet_gaussian(
        image_cloud, sensor_cloud, args.model, args.stats, args.top
    )

    table = Table(title=_t("PointNet-Gaussian match results"))
