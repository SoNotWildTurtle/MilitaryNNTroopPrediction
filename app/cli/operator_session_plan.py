...
def _task_from_recommended_action(action: Mapping[str, Any], index: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "why": _first_text(action.get("detail"), action.get("remediation"), default="Generated from CI triage guidance."),
        "done_when": _first_text(action.get("remediation"), default="The target completes cleanly and artifacts are regenerated."),
    ...
    for key in ("priority_items", "follow_up_items", "risks", "warnings"):
        for index, item in enumerate(_as_list(release_notes.get(key)), start=1):
            if isinstance(item, Mapping):
                title = _first_text(item.get("title"), item.get("name"), item.get("reason"), default=f"{key} {index}")
                detail = _first_text(item.get("detail"), item.get("summary"), item.get("remediation"), default="Review release notes item.")
            else:
                title = str(item)
...
def _handoff_tasks(handoff: Mapping[str, Any]) -> List[Dict[str, Any]]:
    ...
    for key in ("review_steps", "next_steps", "checklist", "handoff_items"):
        for index, item in enumerate(_as_list(handoff.get(key)), start=1):
            if isinstance(item, Mapping):
                title = _first_text(item.get("title"), item.get("task"), item.get("name"), default=f"{key} {index}")
                command = _first_text(item.get("command"), item.get("target"), default="make verify")
                detail = _first_text(item.get("detail"), item.get("why"), item.get("description"), default="Review handoff item.")
            else:
                title = str(item)
                command = "make verify"
...
def build_operator_session_plan(
    ...
    health_summary = triage_summary.get("health_summary", {}) if isinstance(triage_summary.get("health_summary", {}), Mapping) else {}
