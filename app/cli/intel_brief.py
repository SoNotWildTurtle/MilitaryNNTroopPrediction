...
def _render_automation_deployment(payload: Optional[Dict[str, Any]]) -> None:
    if not payload:
        return

    console.print(_t("Automation deployment posture"), style="bold")
    ...
    score = payload.get("deployment_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Deployment score"), f"{float(score):.1f}")

    severity = payload.get("severity_index")
    if isinstance(severity, (int, float)):
        table.add_row(_t("Severity index"), str(int(severity)))

    window = payload.get("deployment_window_hours")
    if isinstance(window, (int, float)):
        table.add_row(_t("Deployment window"), f"{float(window):.2f} {_t('hours')}")

    tracks = payload.get("deployment_tracks")
    if isinstance(tracks, list) and tracks:
        table.add_row(_t("Deployment tracks"), str(len(tracks)))

    console.print(table)

    if isinstance(tracks, list) and tracks:
        track_table = Table(title=_t("Deployment tracks"))
        track_table.add_column(_t("Track"))
        track_table.add_column(_t("Owner"))
        track_table.add_column(_t("Readiness"))
        track_table.add_column(_t("Window (hr)"), justify="right")
        for entry in tracks:
            if not isinstance(entry, dict):
                continue
            window_value = entry.get("window_hours")
            track_table.add_row(
                str(entry.get("name", "-")),
                str(entry.get("owner", "-")) if entry.get("owner") else "-",
                str(entry.get("readiness", "-")),
                f"{float(window_value):.2f}" if isinstance(window_value, (int, float)) else "-",
            )
        console.print(track_table)
    ...
    for label, key in [
        (_t("Drivers:"), "drivers"),
        (_t("Blockers:"), "blockers"),
        (_t("Prerequisites:"), "prerequisites"),
        (_t("Watch items:"), "watch_items"),
    ]:
        values = payload.get(key)
        if isinstance(values, list) and values:
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Deployment actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")


def _render_automation_overwatch(payload: Optional[Dict[str, Any]]) -> None:
    if not payload:
        return

    console.print(_t("Automation overwatch posture"), style="bold")
    ...
    score = payload.get("overwatch_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Overwatch score"), f"{float(score):.1f}")
    ...
    watch_teams = payload.get("watch_teams")
    if isinstance(watch_teams, list) and watch_teams:
        table.add_row(_t("Watch teams"), str(len(watch_teams)))

    fallback = payload.get("fallback_channels")
    if isinstance(fallback, list) and fallback:
        table.add_row(_t("Fallback channels"), str(len(fallback)))

    console.print(table)

    if isinstance(watch_teams, list) and watch_teams:
        console.print(_t("Watch teams:"), style="bold")
    ...
    watch_focus = payload.get("watch_focus")
    if isinstance(watch_focus, list) and watch_focus:
        console.print(_t("Focus areas:"), style="bold")
        for focus in watch_focus:
    ...
    if isinstance(fallback, list) and fallback:
        console.print(_t("Fallback channels:"), style="bold")
        for channel in fallback:
            console.print(f"- {channel}")

    prompts = payload.get("ukrainian_watch_prompts")
    if isinstance(prompts, list) and prompts:
        console.print(_t("Prompts for Ukrainian operators:"), style="bold blue")
        for prompt in prompts:
            console.print(f"- {prompt}", style="blue")
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Overwatch actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")


def _render_automation_battle_management(payload: Optional[Dict[str, Any]]) -> None:
    if not payload:
        return

    console.print(_t("Automation battle management"), style="bold")
    ...
    score = payload.get("battle_management_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Battle management score"), f"{float(score):.1f}")

    severity = payload.get("severity_index")
    if isinstance(severity, (int, float)):
        table.add_row(_t("Severity index"), str(int(severity)))

    window = payload.get("battle_management_window_hours")
    if isinstance(window, (int, float)):
        table.add_row(_t("Coordination window"), f"{float(window):.2f} {_t('hours')}")

    tracks = payload.get("coordination_tracks")
    if isinstance(tracks, list) and tracks:
        table.add_row(_t("Coordination tracks"), str(len(tracks)))

    channels = payload.get("battle_channels")
    if isinstance(channels, list) and channels:
        table.add_row(_t("Battle channels"), str(len(channels)))

    feeds = payload.get("priority_feeds")
    if isinstance(feeds, list) and feeds:
        table.add_row(_t("Priority feeds"), str(len(feeds)))

    console.print(table)

    if isinstance(tracks, list) and tracks:
        track_table = Table(title=_t("Coordination tracks"))
        track_table.add_column(_t("Track"))
        track_table.add_column(_t("Lead"))
        track_table.add_column(_t("Readiness"))
        track_table.add_column(_t("Window (hr)"), justify="right")
        track_table.add_column(_t("Status"))
        for track in tracks:
            if not isinstance(track, dict):
                continue
            window_value = track.get("window_hours")
            track_table.add_row(
                str(track.get("name", "-")),
                str(track.get("lead", "-")) if track.get("lead") else "-",
                str(track.get("readiness", "-")) if track.get("readiness") else "-",
                f"{float(window_value):.2f}" if isinstance(window_value, (int, float)) else "-",
                str(track.get("status", "-")) if track.get("status") else "-",
            )
        console.print(track_table)

    for label, key in [
        (_t("Drivers:"), "drivers"),
        (_t("Focus areas:"), "focus_areas"),
        (_t("Watch items:"), "watch_items"),
        (_t("Priority feeds:"), "priority_feeds"),
        (_t("Integration targets:"), "integration_targets"),
        (_t("Battle channels:"), "battle_channels"),
        (_t("Handoff requirements:"), "handoff_requirements"),
    ]:
        values = payload.get(key)
        if isinstance(values, list) and values:
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Battle management actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")


def _render_automation_campaign_orchestration(payload: Optional[Dict[str, Any]]) -> None:
    if not payload:
        return

    console.print(_t("Automation campaign orchestration"), style="bold")
    ...
    score = payload.get("campaign_orchestration_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Orchestration score"), f"{float(score):.1f}")

    severity = payload.get("severity_index")
    if isinstance(severity, (int, float)):
        table.add_row(_t("Severity index"), str(int(severity)))

    window = payload.get("campaign_window_hours")
    if isinstance(window, (int, float)):
        table.add_row(_t("Next window"), f"{float(window):.2f} {_t('hours')}")

    tracks = payload.get("orchestration_tracks")
    if isinstance(tracks, list) and tracks:
        table.add_row(_t("Tracks"), str(len(tracks)))

    partners = payload.get("integration_partners")
    if isinstance(partners, list) and partners:
        table.add_row(_t("Integration partners"), str(len(partners)))

    channels = payload.get("campaign_channels")
    if isinstance(channels, list) and channels:
        table.add_row(_t("Channels"), str(len(channels)))

    dependencies = payload.get("operational_dependencies")
    if isinstance(dependencies, list) and dependencies:
        table.add_row(_t("Dependencies"), str(len(dependencies)))

    console.print(table)

    if isinstance(tracks, list) and tracks:
        track_table = Table(title=_t("Orchestration tracks"))
        track_table.add_column(_t("Track"))
        track_table.add_column(_t("Lead"))
        track_table.add_column(_t("Mode"))
        track_table.add_column(_t("Readiness"))
        track_table.add_column(_t("Window (hr)"), justify="right")
        track_table.add_column(_t("Status"))
        track_table.add_column(_t("Source"))
        for track in tracks:
            if not isinstance(track, dict):
                continue
            window_value = track.get("window_hours")
            track_table.add_row(
                str(track.get("name", "-")),
                str(track.get("lead", "-")) if track.get("lead") else "-",
                str(track.get("mode", "-")) if track.get("mode") else "-",
                str(track.get("readiness", "-")) if track.get("readiness") else "-",
                f"{float(window_value):.2f}" if isinstance(window_value, (int, float)) else "-",
                str(track.get("status", "-")) if track.get("status") else "-",
                str(track.get("source", "-")) if track.get("source") else "-",
            )
        console.print(track_table)
    ...
    for label, key in [
        ...
        (_t("Focus areas:"), "focus_areas"),
        (_t("Watch items:"), "watch_items"),
        (_t("Operational dependencies:"), "operational_dependencies"),
        (_t("Integration partners:"), "integration_partners"),
        (_t("Campaign channels:"), "campaign_channels"),
    ]:
        values = payload.get(key)
        if isinstance(values, list) and values:
            console.print(label, style="bold")
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Campaign actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")


def _render_automation_joint_operations(payload: Optional[Dict[str, Any]]) -> None:
    if not payload:
        return

    console.print(_t("Automation joint operations"), style="bold")
    ...
    score = payload.get("joint_operations_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Joint operations score"), f"{float(score):.1f}")

    severity = payload.get("severity_index")
    if isinstance(severity, (int, float)):
        table.add_row(_t("Severity index"), str(int(severity)))

    window = payload.get("joint_window_hours")
    if isinstance(window, (int, float)):
        table.add_row(_t("Next window"), f"{float(window):.2f} {_t('hours')}")

    tracks = payload.get("joint_operation_tracks")
    if isinstance(tracks, list) and tracks:
        table.add_row(_t("Joint tracks"), str(len(tracks)))

    partners = payload.get("coalition_partners")
    if isinstance(partners, list) and partners:
        table.add_row(_t("Coalition partners"), str(len(partners)))

    channels = payload.get("integration_channels")
    if isinstance(channels, list) and channels:
        table.add_row(_t("Integration channels"), str(len(channels)))

    support_cells = payload.get("support_cells")
    if isinstance(support_cells, list) and support_cells:
        table.add_row(_t("Support cells"), str(len(support_cells)))

    console.print(table)

    if isinstance(tracks, list) and tracks:
        track_table = Table(title=_t("Joint operation tracks"))
        track_table.add_column(_t("Track"))
        track_table.add_column(_t("Lead"))
        track_table.add_column(_t("Mode"))
        track_table.add_column(_t("Readiness"))
    ...
    for label, key in [
        ...
        (_t("Watch items:"), "watch_items"),
        (_t("Operational dependencies:"), "operational_dependencies"),
        (_t("Coalition partners:"), "coalition_partners"),
        (_t("Integration channels:"), "integration_channels"),
        (_t("Support cells:"), "support_cells"),
        (_t("Handoff requirements:"), "handoff_requirements"),
    ]:
        values = payload.get(key)
        if isinstance(values, list) and values:
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Joint operation actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")


def _render_automation_theater_command(payload: Optional[Dict[str, Any]]) -> None:
    if not payload:
        return

    console.print(_t("Automation theater command"), style="bold")
    ...
    score = payload.get("theater_command_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Theater command score"), f"{float(score):.1f}")

    severity = payload.get("severity_index")
    if isinstance(severity, (int, float)):
        table.add_row(_t("Severity index"), str(int(severity)))

    window = payload.get("command_window_hours")
    if isinstance(window, (int, float)):
        table.add_row(_t("Next command window"), f"{float(window):.2f} {_t('hours')}")

    tracks = payload.get("command_tracks")
    if isinstance(tracks, list) and tracks:
        table.add_row(_t("Command tracks"), str(len(tracks)))

    theaters = payload.get("coordinating_theaters")
    if isinstance(theaters, list) and theaters:
        table.add_row(_t("Coordinating theatres"), str(len(theaters)))

    commanders = payload.get("coalition_commanders")
    if isinstance(commanders, list) and commanders:
        table.add_row(_t("Coalition commanders"), str(len(commanders)))

    requirements = payload.get("support_requirements")
    if isinstance(requirements, list) and requirements:
        table.add_row(_t("Support requirements"), str(len(requirements)))

    channels = payload.get("command_channels")
    if isinstance(channels, list) and channels:
        table.add_row(_t("Command channels"), str(len(channels)))

    console.print(table)

    if isinstance(tracks, list) and tracks:
        track_table = Table(title=_t("Theater command tracks"))
        track_table.add_column(_t("Track"))
        track_table.add_column(_t("Lead"))
        track_table.add_column(_t("Mode"))
        track_table.add_column(_t("Readiness"))
    ...
    for label, key in [
        ...
        (_t("Focus areas:"), "focus_areas"),
        (_t("Watch items:"), "watch_items"),
        (_t("Coordinating theatres:"), "coordinating_theaters"),
        (_t("Coalition commanders:"), "coalition_commanders"),
        (_t("Command channels:"), "command_channels"),
        (_t("Support requirements:"), "support_requirements"),
    ]:
        values = payload.get(key)
        if isinstance(values, list) and values:
            console.print(label, style="bold")
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Command actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")


def _render_automation_supreme_command(payload: Optional[Dict[str, Any]]) -> None:
    if not payload:
        return

    console.print(_t("Automation supreme command"), style="bold")
    ...
    score = payload.get("supreme_command_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Supreme command score"), f"{float(score):.1f}")
    ...
    nodes = payload.get("command_nodes")
    if isinstance(nodes, list) and nodes:
        table.add_row(_t("Command nodes"), str(len(nodes)))

    liaisons = payload.get("coalition_liaisons")
    if isinstance(liaisons, list) and liaisons:
        table.add_row(_t("Coalition liaisons"), str(len(liaisons)))

    dependencies = payload.get("support_dependencies")
    if isinstance(dependencies, list) and dependencies:
        table.add_row(_t("Support dependencies"), str(len(dependencies)))
    ...
    if isinstance(tracks, list) and tracks:
        track_table = Table(title=_t("Supreme command tracks"))
        track_table.add_column(_t("Track"))
        track_table.add_column(_t("Lead"))
        track_table.add_column(_t("Mode"))
        track_table.add_column(_t("Readiness"))
    ...
    for label, key in [
        ...
        (_t("Focus areas:"), "focus_areas"),
        (_t("Watch items:"), "watch_items"),
        (_t("Command nodes:"), "command_nodes"),
        (_t("Integration channels:"), "integration_channels"),
        (_t("Coalition liaisons:"), "coalition_liaisons"),
        (_t("Support dependencies:"), "support_dependencies"),
    ]:
        values = payload.get(key)
        if isinstance(values, list) and values:
            console.print(label, style="bold")
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Supreme command actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")


def _render_automation_force_projection(payload: Optional[Dict[str, Any]]) -> None:
    if not payload:
        return

    console.print(_t("Automation force projection"), style="bold")
    ...
    score = payload.get("force_projection_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Force projection score"), f"{float(score):.1f}")

    severity = payload.get("severity_index")
    if isinstance(severity, (int, float)):
        table.add_row(_t("Severity index"), str(int(severity)))

    window = payload.get("projection_window_hours")
    if isinstance(window, (int, float)):
        table.add_row(
            _t("Next projection window"), f"{float(window):.2f} {_t('hours')}"
        )

    packages = payload.get("force_packages")
    if isinstance(packages, list) and packages:
        table.add_row(_t("Force packages"), str(len(packages)))

    tracks = payload.get("projection_tracks")
    if isinstance(tracks, list) and tracks:
        table.add_row(_t("Projection tracks"), str(len(tracks)))

    channels = payload.get("projection_channels")
    if isinstance(channels, list) and channels:
        table.add_row(_t("Projection channels"), str(len(channels)))

    nodes = payload.get("projection_nodes")
    if isinstance(nodes, list) and nodes:
        table.add_row(_t("Projection nodes"), str(len(nodes)))
    ...
    if isinstance(tracks, list) and tracks:
        track_table = Table(title=_t("Projection tracks"))
        track_table.add_column(_t("Track"))
        track_table.add_column(_t("Lead"))
        track_table.add_column(_t("Mode"))
        track_table.add_column(_t("Readiness"))
    ...
    for label, key in [
        (_t("Drivers:"), "drivers"),
        (_t("Watch items:"), "watch_items"),
        (_t("Force packages:"), "force_packages"),
        (_t("Projection channels:"), "projection_channels"),
        (_t("Projection nodes:"), "projection_nodes"),
        (_t("Support dependencies:"), "support_dependencies"),
    ]:
        values = payload.get(key)
        if isinstance(values, list) and values:
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Force projection actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")
...
def _render_operator_dashboard(payload: Optional[Dict[str, Any]]) -> None:
    ...
    alerts = payload.get("operator_alerts")
    if isinstance(alerts, list) and alerts:
        console.print(_t("Operator alerts:"), style="bold red")
        for alert in alerts:
    ...
    checklist = payload.get("shift_checklist")
    if isinstance(checklist, list) and checklist:
        console.print(_t("Shift checklist:"), style="bold")
        for item in checklist:
            console.print(f"- {item}")

    shortcuts = payload.get("utility_shortcuts")
    if isinstance(shortcuts, list) and shortcuts:
        table = Table(title=_t("Operator utilities"))
        table.add_column(_t("Shortcut"))
        table.add_column(_t("Command"))
        ...
        for shortcut in shortcuts:
            ...
            description = str(shortcut.get("description", "")).strip()
            if not label or not command:
                continue
            table.add_row(label, command, description or "-")
        console.print(table)
...
def _render_automation_strategic_convergence(payload: Optional[Dict[str, Any]]) -> None:
    ...
    console.print(_t("Automation strategic convergence"), style="bold")
    ...
    score = payload.get("strategic_convergence_score")
    if isinstance(score, (int, float)):
        table.add_row(_t("Strategic convergence score"), f"{float(score):.1f}")

    severity = payload.get("severity_index")
    if isinstance(severity, (int, float)):
        table.add_row(_t("Severity index"), str(int(severity)))

    window = payload.get("next_convergence_window_hours")
    if isinstance(window, (int, float)):
        table.add_row(
            _t("Next convergence window"), f"{float(window):.2f} {_t('hours')}"
        )

    tracks = payload.get("cross_domain_tracks")
    if isinstance(tracks, list) and tracks:
        table.add_row(_t("Cross-domain tracks"), str(len(tracks)))

    nodes = payload.get("national_command_nodes")
    if isinstance(nodes, list) and nodes:
        table.add_row(_t("National command nodes"), str(len(nodes)))

    partners = payload.get("coalition_partners")
    if isinstance(partners, list) and partners:
        table.add_row(_t("Coalition partners"), str(len(partners)))

    channels = payload.get("strategic_channels")
    if isinstance(channels, list) and channels:
        table.add_row(_t("Strategic channels"), str(len(channels)))

    console.print(table)

    if isinstance(tracks, list) and tracks:
        track_table = Table(title=_t("Cross-domain automation tracks"))
        track_table.add_column(_t("Track"))
        track_table.add_column(_t("Lead"))
        track_table.add_column(_t("Mode"))
        track_table.add_column(_t("Readiness"))
    ...
    for label, key in [
        ...
        (_t("Focus areas:"), "focus_areas"),
        (_t("Watch items:"), "watch_items"),
        (_t("Strategic dependencies:"), "strategic_dependencies"),
        (_t("Support requirements:"), "support_requirements"),
        (_t("Strategic channels:"), "strategic_channels"),
        (_t("National command nodes:"), "national_command_nodes"),
        (_t("Coalition partners:"), "coalition_partners"),
    ]:
        values = payload.get(key)
        if isinstance(values, list) and values:
    ...
    actions = payload.get("recommended_actions")
    if isinstance(actions, list) and actions:
        console.print(_t("Strategic automation actions:"), style="bold")
        for action in actions:
            console.print(f"- {action}")
...
def main(area: Optional[str], hours: int, limit: int, raw: bool) -> None:
    ...
    if brief.get("automation_deployment"):
        _render_automation_deployment(brief["automation_deployment"])
    if brief.get("automation_overwatch"):
        _render_automation_overwatch(brief["automation_overwatch"])
    if brief.get("automation_battle_management"):
        _render_automation_battle_management(brief["automation_battle_management"])
    if brief.get("automation_campaign_orchestration"):
        _render_automation_campaign_orchestration(
            brief["automation_campaign_orchestration"]
        )
    if brief.get("automation_theater_command"):
        _render_automation_theater_command(brief["automation_theater_command"])
    if brief.get("automation_supreme_command"):
        _render_automation_supreme_command(brief["automation_supreme_command"])
    if brief.get("automation_strategic_convergence"):
        _render_automation_strategic_convergence(
            brief["automation_strategic_convergence"]
        )
