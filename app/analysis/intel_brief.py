...
def _derive_operator_dashboard_snapshot(brief: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    checklist: List[str] = []
    watch_items: List[str] = []
    utility_shortcuts: List[Dict[str, str]] = []
    cards: List[Dict[str, Any]] = []

    def add_shortcut(label: str, command: str, description: str) -> None:
        if not label or not command:
            return
        utility_shortcuts.append(
            {
                "label": label,
                "command": command,
    ...
    if pressure_status in {"critical_backlog", "prediction_gap"}:
        ...
        alerts.append("Prediction backlog is critical; clear queues to restore operator flow.")
        checklist.append("Coordinate surge analysts to triage predictions within the next 2 hours.")
        add_shortcut(
            "Surge reroute",
            "python -m app.cli.dashboard",
            "Open the operator dashboard and enable surge routing for prediction queues.",
        )
    elif pressure_status in {"backlog", "prediction_gap_watch", "feedback_strain", "quality_watch"}:
        score -= 10
        drivers.append("Operator backlog is building; monitor queues and modelling coverage.")
        watch_items.append("Prediction queues growing; verify modelling coverage before shift end.")
        add_shortcut(
            "Queue monitor",
            "python -m app.cli.intel_brief --hours 6 --json",
            "Export a JSON brief to share backlog and modelling gaps with the next shift.",
        )
    ...
    if resilience_status in {"critical", "degraded"}:
        ...
        alerts.append("Resilience is degraded; prepare manual fallback paths for operators.")
    ...
    if continuity_status in {"at_risk", "constrained"}:
        ...
        checklist.append("Run a quick continuity drill for priority operator tracks this shift.")
    ...
    if assurance_status in {"blocked", "critical"}:
        ...
        alerts.append("Mission assurance blockers are active; confirm leadership ownership.")
    ...
    if isinstance(confidence_score, (float, int)):
        ...
        if confidence_score < 60:
            ...
            alerts.append("Telemetry confidence is low; pair operators with analysts for validation.")
    ...
    if mission_control_status in {"manual_bridge", "degraded"} or autonomy_status in {"manual", "guarded"}:
        ...
        alerts.append("Automation is under manual control; enforce supervised releases this shift.")
        checklist.append("Log automation overrides in the duty journal and brief the next team.")
        add_shortcut(
            "Mission control console",
            "python -m app.cli.control_center",
            "Launch the control centre to supervise automation overrides with leadership.",
        )
    elif mission_control_status in {"mission_ready", "supervised"} and autonomy_status in {"mission_ready", "autonomous_ready", "autonomous"}:
        score += 4
        watch_items.append("Automation mission control steady; maintain routine spot checks.")

    guardrails_status = str(guardrails.get("status", "")).lower()
    if guardrails_status:
        cards.append({"label": "Guardrails", "value": guardrails_status})
    if guardrails_status in {"manual_guarded", "locked_down", "manual"}:
        score -= 6
        drivers.append("Automation guardrails are tightened; expect slower hand-offs.")
        checklist.append("Confirm dual-approval steps with the duty officer for automation releases.")
        add_shortcut(
            "Guardrail audit",
            "python -m app.cli.system_check --skip-optional",
            "Run the system check to validate guardrail prerequisites before enabling automation.",
        )
    ...
    if isinstance(feeds, dict) and feeds:
        ...
        if stale:
            ...
            alerts.append("Stale telemetry detected; switch to backup validation for priority feeds.")
            watch_items.extend([f"{name} feed stale" for name in stale])
            add_shortcut(
                "Telemetry validation",
                "python -m app.cli.system_check --skip-optional",
                "Verify degraded feeds and log findings for operators in the duty journal.",
            )
        if warn:
            score -= 4
            drivers.append(f"Telemetry freshness warnings for: {', '.join(sorted(warn))}.")
            watch_items.extend([f"{name} feed warning" for name in warn])

    support_status = str(support.get("status", "")).lower()
    if support_status in {"mobilise", "strained"}:
        score -= 6
        drivers.append("Support teams are mobilising, which may delay operator requests.")
        alerts.append("Support teams are mobilising; operator requests may queue.")
        checklist.append("Confirm support ticket routing for operator asks and escalation paths.")
    elif support_status in {"reinforce", "steady"}:
        watch_items.append("Support posture steady; maintain SLA spot checks with operators.")

    add_shortcut(
        "Operator dashboard",
        "python -m app.cli.dashboard",
        "Open the dashboard for live maps, telemetry alerts, and automation controls.",
    )
    add_shortcut(
        "Shift handoff pack",
        "python -m app.cli.intel_brief --hours 6 --json",
        "Generate a machine-readable brief to attach to shift logs or Signal threads.",
    )
    add_shortcut(
        "Control centre",
        "python -m app.cli.control_center",
        "Present the leadership control centre with live risk, posture, and SMS alerts.",
    )
    add_shortcut(
        "Data checks",
        "python -m app.cli.system_check --skip-optional",
        "Validate critical dependencies and telemetry endpoints before tasking crews.",
    )
    ...
    watch_items = list(dict.fromkeys(watch_items))
    seen_shortcuts: Dict[Tuple[str, str], bool] = {}
    deduped_shortcuts: List[Dict[str, str]] = []
    for shortcut in utility_shortcuts:
        label = str(shortcut.get("label", "")).strip()
        command = str(shortcut.get("command", "")).strip()
        desc = str(shortcut.get("description", "")).strip()
        key = (label, command)
        if not label or not command or key in seen_shortcuts:
            continue
        seen_shortcuts[key] = True
        deduped_shortcuts.append({"label": label, "command": command, "description": desc})
    ...
    if deduped_shortcuts:
        snapshot["utility_shortcuts"] = deduped_shortcuts
    if prompts:
        snapshot["ukrainian_operator_prompts"] = prompts
    if alerts:
        snapshot["operator_alerts"] = alerts
    if watch_items:
        snapshot["watch_items"] = watch_items
    if checklist:
        snapshot["shift_checklist"] = checklist
    if summary:
...
def gather_intelligence_brief(
    *,
    ...
    if dashboard:
        ...
        if dashboard.get("operator_alerts"):
            insight["alert_count"] = len(dashboard.get("operator_alerts", []))
        if dashboard.get("shift_checklist"):
            insight["checklist_items"] = len(dashboard.get("shift_checklist", []))
        shortcuts = dashboard.get("utility_shortcuts")
        if isinstance(shortcuts, list) and shortcuts:
            insight["utility_shortcuts"] = len(shortcuts)
