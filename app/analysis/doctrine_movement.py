...
# preserve insertion order so downstream helpers can offer friendly menus
DEFAULT_LEVELS: Tuple[str, ...] = tuple(_LEVEL_FIELDS.keys())

# exported constant for menu helpers and API clients
AVAILABLE_LEVELS: Tuple[str, ...] = DEFAULT_LEVELS
...
def doctrine_movement_hierarchy(
    *,
    levels: Optional[Sequence[str]] = None,
    identifiers: Optional[Mapping[str, Optional[str]]] = None,
    bucket: str = "hour",
    hours: int = 24,
    start: Optional[datetime] = None,
    end: Optional[datetime] = None,
) -> Dict[str, object]:
    """Return doctrine drilldowns for multiple hierarchy levels at once.

    Parameters mirror :func:`doctrine_movement_drilldown` but accept a list of
    hierarchy ``levels`` and optional per-level ``identifiers``. Results are
    grouped in a mapping keyed by level name so callers can render nested
    drilldowns or build cascading menus without issuing repeated queries.
    """

    chosen_levels: Tuple[str, ...]
    if levels is None:
        chosen_levels = DEFAULT_LEVELS
    else:
        chosen_levels = tuple(level.lower() for level in levels)
        for lvl in chosen_levels:
            if lvl not in _LEVEL_FIELDS:
                raise ValueError(
                    f"Unsupported level '{lvl}'. Expected one of {DEFAULT_LEVELS}"
                )

    results: Dict[str, Dict[str, object]] = {}
    for lvl in chosen_levels:
        identifier = identifiers.get(lvl) if identifiers else None
        results[lvl] = doctrine_movement_drilldown(
            lvl,
            identifier,
            bucket=bucket,
            hours=hours,
