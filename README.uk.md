...
- `cli/lag_report.py` – кореляції зі зсувом у часі.
 - `cli/fusion_report.py` – запуск виявлень камери, LIDAR та Bluetooth для військ, техніки й дронів і показ злитих результатів; приймає `--image`, `--pointcloud` та `--bluetooth` для введення файлів без підказок.
