...
- `satellite/` – завантаження зображень Sentinel та конвеєр інференсу
  ...
  - `pipeline/realtime.py` – збереження виявлень і прогнозів у MongoDB
    ...
    - `detection/lidar_detector.py` – виявлення військ, техніки та дронів за LIDAR‑даними
...
Модуль LIDAR забезпечує електромагнітне виявлення, а модуль злиття сенсорів поєднує результати камер і LIDAR для підвищення впевненості.
...
- `cli/fusion_report.py` – запуск виявлень камери та LIDAR для військ, техніки й дронів і показ злитих результатів; приймає `--image` та `--pointcloud` для введення файлів без підказок.
...
python -m app.cli.fusion_report --image sample.jpg --pointcloud sample.pcd  # злиття виявлень камери й LIDAR для військ, техніки й дронів
