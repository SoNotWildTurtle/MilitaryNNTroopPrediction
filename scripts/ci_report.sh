...
- operator-readiness.md: operator-facing go/no-go checklist generated from diagnostics.
- operator-readiness.json: machine-readable operator readiness checklist.
...
# Release notes, triage summaries, reviewer handoffs, and operator checklists read
# the manifest, while the manifest must also include their final outputs. A
# multi-pass handoff keeps human notes, narrow rerun guidance, copyable review
# context, operator go/no-go status, and the final machine-readable manifest
# useful without requiring any network calls.
